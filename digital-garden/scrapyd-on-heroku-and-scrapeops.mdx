...
In this case, the `requirements.txt` would contain the depedencies for both Scrapyd and the Scrapy project, so you won't need to use the `--include-deps` flag when deploying the project.
 
I don't really recommend this because combining config files makes things more confusing with little benefit. (The `--inlcude-deps` flag is new so this option made more sense previously before it was possible to manage the dependencies separately.)  Having your Scrapy project in the same directory as your Scrapyd server doesn't magically associate your Scrapy project with your Scrapyd server - you still need to deploy your Scrapy project to Scrapy (Step 3 below).
